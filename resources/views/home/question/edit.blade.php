...
@section('content')
    <div class="row">
        <div class="col-md-8 col-12">
            <div class="mb-3 back">
                <a class="text-secondary" href="{{ route('question.show', [$question->id, $question->slug]) }}"><h6><i class="fa-solid fa-arrow-left"></i> Back to main question</h6></a>
            </div>

            <div class="your-question ask-question">
                <h5>Edit Your Question</h5>
                @if ($errors->any())
                ...
                <div class="card mt-3">
                    <div class="card-body">
                        <form action="{{ route('question.update', [$question->id, $question->slug]) }}" method="post" class="ask-question">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ $question->title }}" placeholder="e.g. Who's main character in anime One Piece ?" required>
                                @error('title')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="body" id="summernote" cols="100" rows="10">{!! $question->body !!}</textarea>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control tag" id="tag" placeholder="e.g. (anime, one-piece).">
                                <div class="col-md-12 mb-4 d-none">
                                    <select name="tag[]" id="tag-result" class="multipleselect" multiple="multiple">
                                        @foreach ($question['tag'] as $tag)
                                            <option id="tag-option-{{ $tag['tag'] }}" value="{{ $tag['tag'] }}" selected>{{ $tag['tag'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="press-space"></div>
                                <div class="tag-info">
                                    <p class="m-0 mt-2"><strong>Tags :</strong></p>
                                </div>
                                <div class="tag-result mt-1 mb-4">
                                    @foreach ($question['tag'] as $tag)
                                        <span class="tag-span ask-question-tag mr-2"><i class="fa-solid fa-xmark"></i>{{ $tag['tag'] }}</span>
                                    @endforeach
                                </div>
                            </div>
...
@push('script')
    <script>
        ...
        $('.tag-result').on('click', '.tag-span', function(){
            ...
            url = url.replace(':id', tagOption.innerHTML);
            tagOption.remove();
            deleteTag(url);
        });
